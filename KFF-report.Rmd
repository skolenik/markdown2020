...
```{r read_spss}
KFF_data <- haven::read_spss(here("data","Kaiser Health Tracker Poll_April_Final no rotates.sav"))
```

```{r vtable, warning=FALSE}
...
q1_results %>% filter( category < 5) %>%
  ...
  as_tibble() -> q1_results_toplot
if (is_html_output()) {
  q1_results_toplot %>% select(item, better_label, estimate, Responses) %>% kable(digits=2) %>%
...
#  scale_fill_brewer(palette='Blues',
  ...
  ylab("Percentage") + xlab("Party affiliation")
q6plot
```
